...
const typeColors = {
  ...
  dark: "#705848",
  steel: "#B8B8D0",
  fairy: "#EE99AC"  // Fixed typo here, was "dark" twice
};
...
function setTypeBackgroundColor(pokemon) {
  ...
  // Update power type background colors
  const powerElements = document.querySelectorAll(".power-wrapper > p");
  powerElements.forEach(element => {
    element.style.backgroundColor = color;
  });

  // Update stat text colors
  const statElements = document.querySelectorAll(".stats-wrap p.stats");
  statElements.forEach(element => {
    element.style.color = color;
  });

  // Update progress bars
  const progressBars = document.querySelectorAll(".stats-wrap .progress-bar");
  const rgbaColor = rgbaFromHex(color);
  
  // Remove any previous style tags to prevent duplication
  const oldStyle = document.getElementById("pokemon-progress-style");
  if (oldStyle) {
    oldStyle.remove();
  }
  
  const styleTag = document.createElement("style");
  styleTag.id = "pokemon-progress-style";
  styleTag.innerHTML = `
